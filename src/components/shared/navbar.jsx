...
export function Navbar() {
  ...
  // Desktop Navigation Items (Logged In)
  ...
  // Desktop Navigation Items (Guest)
  const guestDesktopNavItems = [
    { icon: Home, label: "Home", href: "/", active: pathname === "/" },
    { icon: Award, label: "Features", href: "/features", active: pathname === "/features" },
    { icon: Briefcase, label: "Pricing", href: "/pricing", active: pathname === "/pricing" },
  ]

  // Mobile Bottom Tab Items (Logged In) - NO TOP NAVBAR
  const userMobileTabItems = [
    { icon: Home, label: "Home", href: "/", active: pathname === "/" },
    { icon: Search, label: "Explore", href: "/explore", active: pathname === "/explore" },
    { icon: Briefcase, label: "Jobs", href: "/jobs", active: pathname === "/jobs" },
    { icon: GraduationCap, label: "Courses", href: "/courses", active: pathname === "/courses" },
  ...
  // Mobile Bottom Tab Items (Guest)
  const guestMobileTabItems = [
    { icon: Home, label: "Home", href: "/", active: pathname === "/" },
    { icon: Award, label: "Features", href: "/features", active: pathname === "/features" },
    { icon: Briefcase, label: "Pricing", href: "/pricing", active: pathname === "/pricing" },
    { icon: User, label: "Sign In", href: "/login", active: pathname === "/login" },
  ]

  const desktopNavItems = user ? userDesktopNavItems : guestDesktopNavItems
  const mobileTabItems = user ? userMobileTabItems : guestMobileTabItems
  ...
  return (
    <>
      {/* DESKTOP NAVBAR - ONLY FOR DESKTOP */}
      ...
      {/* MOBILE VIEW - NO TOP NAVBAR, ONLY BOTTOM BAR + MENU ICON */}
      <div className="md:hidden">
        {/* Only Menu Button (3 lines) at top right - NO NAVBAR */}
        {user && (
          <div className="fixed top-0 right-0 z-50 p-3">
            <button
              onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
              className="flex items-center justify-center w-9 h-9 rounded-xl bg-white/80 dark:bg-slate-900/80 backdrop-blur-md shadow-md active:scale-95 transition-all duration-200 border border-slate-200/50 dark:border-slate-800/50"
            >
              {mobileMenuOpen ? (
        ...
        {/* No spacer needed for logged in users since no top navbar */}

        {/* Premium Bottom Tab Bar - Only Navigation for Mobile */}
        {!keyboardVisible && (
          <div className="fixed bottom-0 left-0 right-0 z-50 bg-white/80 dark:bg-slate-950/80 backdrop-blur-2xl border-t border-white/20 dark:border-white/10 rounded-t-2xl shadow-2xl safe-bottom">
              ...
              {mobileTabItems.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className={`text-[10px] font-medium ${isActive ? 'text-blue-600 dark:text-blue-400 font-semibold' : 'opacity-70'}`}>
                      {item.label}
                    </span>
                  </Link>
                )
      ...
      {/* Mobile Bottom Sheet Menu (Settings, Theme, Profile, etc.) */}
      <div 
